...
??? note "Key Takeaway" - 基于派生类调用函数时，编译器沿着继承链向上查找并使用找到的第一个
...
当  `derived.identify()`  执行 s 时，它被解析为 `Derived::identify()`。但是它做的第一件事是调用 `Base::identify()`，进而打印 “I am a Base”。当 `Base::identify()` 返回时，`Derived::identify()` 会继续执行并打印 “I am a Derived”。
