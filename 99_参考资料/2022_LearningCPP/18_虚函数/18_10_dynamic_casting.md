...
因为 C++中没有“空引用”，所有 `dynamic_cast` 在失败时不能返回空引用。所以，如果引用的 `dynamic_cast` 失败，则会抛出`std::bad_cast` 类型的异常。我们将在本教程后面讨论异常。

## `dynamic_cast` vs `static_cast`

新手程序员有时会对何时使用`static_cast`和`dynamic_cast`感到困惑。答案很简单：除非是向下类型转换（在这种情况下，dynamic_cast 通常是更好的选择），否则一律使用 static_cast。但是，你还应该考虑完全避免强制转换，而只使用虚函数。
...
[[Run-time-type-information-RTTI|运行时类型信息(RTTI)]]是 C++的一个特性，它在运行时会暴露对象数据类型的信息。`dynamic_cast` 利用了特性。因为 RTTI 有相当大的空间性能开销，一些编译器允许你关闭 RTTI 作为一种优化方式。不用说，如果这样做，`dynamic_cast` 将不能正确工作。
