...
不仅如此，任何 const 成员函数如果尝试修改成员变量或者调用非 const 类型的函数，都会导致编译保存，例如：
...
最后，尽管不常用，但我们的确可以通过重载获得同一函数的 const 和非 const 版本。因为 const 限定符被认为是函数签名的一部分，所以当两个函数具有不同的 const 签名的函数会被认为是两个不同的函数。
