...
### Structured bindings
A proposal for de-structuring initialization, that would allow writing `auto [ x, y, z ] = expr;` where the type of `expr` was a tuple-like object, whose elements would be bound to the variables `x`, `y`, and `z` (which this construct declares). _Tuple-like objects_ include [`std::tuple`](02~软件/00~编程语言/CPP-Notes/00~现代%20C++%20版本/99~参考资料/modern-cpp-features/README.md#tuples), `std::pair`, [`std::array`](02~软件/00~编程语言/CPP-Notes/00~现代%20C++%20版本/99~参考资料/modern-cpp-features/README.md#stdarray), and aggregate structures.
```c++
