```md
这篇文章是《现代 C++ 教程：高速上手 C++ 11、14、17、20》中的第 1 章，主要介绍了现代 C++ 的弃用特性和与 C 语言的兼容性。

### 1.1 被弃用的特性

文章首先列举了从 C++11 开始被弃用的主要特性，并解释了弃用的含义。弃用的特性包括但不限于：

- 字符串字面值常量不能赋值给 `char *`，应使用 `const char *` 或 `auto`。
- 异常说明、`unexpected_handler`、`set_unexpected()` 等被弃用，应使用 `noexcept`。
- `auto_ptr` 被弃用，应使用 `unique_ptr`。
- `register` 关键字被弃用。
- `bool` 类型的 `++` 操作被弃用。
- 如果一个类有析构函数，为其生成拷贝构造函数和拷贝赋值运算符的特性被弃用。
- C 语言风格的类型转换被弃用，应使用 `static_cast`、`reinterpret_cast`、`const_cast`。
- C++17 标准中弃用了一些 C 标准库，例如 `<ccomplex>`、`<cstdalign>`、`<cstdbool>` 与 `<ctgmath>`。

### 1.2 与 C 的兼容性

文章接着讨论了 C++ 与 C 的兼容性问题。尽管 C++ 不是 C 的超集，但在某些情况下仍需要使用 C 语言代码。文章通过一个示例代码展示了如何在 C++ 中调用 C 语言代码，并介绍了使用 `extern "C"` 进行分离编译和链接的方法。

### 进一步阅读的参考文献

最后，文章提供了一些进一步阅读的参考文献，包括《C++ 语言导学》、C++ 历史、C++ 特性在 GCC/Clang 等编译器中的支持情况，以及 C++98 与 C99 之间的区别。

这篇文章旨在帮助读者了解现代 C++ 的弃用特性和与 C 语言的兼容性，为后续章节的学习打下基础。
```
...
![图 1.2: C 和 C++ 互相兼容情况](https://ngte-superbed.oss-cn-beijing.aliyuncs.com/item/20230504120058.png)
