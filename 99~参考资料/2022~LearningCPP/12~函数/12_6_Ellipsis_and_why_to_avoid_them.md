...
The next thing we need to do is make list point to our ellipsis parameters. We do this by calling va*start(). va_start() takes two parameters: the va_list itself, and the name of the _last* non-ellipsis parameter in the function. Once va_start() has been called, va_list points to the first parameter in the ellipsis.
