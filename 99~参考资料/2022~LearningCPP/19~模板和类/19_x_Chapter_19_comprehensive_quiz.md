...
- 使用[[02~软件/10~大前端/Vue-Notes/02~语法基础/01~模板与渲染/template|模板]]可以基于[[placeholder-types|占位符类型]]来定义函数或者类，这样我们就可以定义出配合不同类型工作，但是结构完全一致的函数或者类。被实例化的函数或者类，称为[[function-instance|函数实例]]或[[class-instance|类实例]]
